...
public class DivineHelmetRenderer<E extends LivingEntity, M extends HumanoidModel<E>> extends RenderLayer<E, M> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation("divine"), "3");
    private static final ResourceLocation TEXTURE = new ResourceLocation("textures/models/armor/divine_layer_3.png");
    private final EntityModelSet model;

    public DivineHelmetRenderer(RenderLayerParent<E, M> renderer, EntityModelSet modelSet) {
        super(renderer);
        this.model = modelSet;
    }
    ...
    public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, E pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if(stack.is(ModItems.DIVINE_HELMET.get())) {
            pPoseStack.pushPose();
